        ...
        for cmt in data['Comment']:
            ...
            if cmt['Number reply'] != 0:
                ...
                for i,rep in enumerate(cmt['Reply cmt']):
                    pre += [parent["Cmt"] for parent in cmt['Reply cmt'][:i]]
                    Result.append(assignLabel(data['Content'], pre, rep["Cmt"]))
            print("Xong")
    return Result
...
for filePath in listFile:
    df = pd.DataFrame(cleanData(filePath), columns=['Poster', 'Pre-Comment', 'Comment', 'Label'])
    print(df)
